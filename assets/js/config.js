...
// ===== API CONFIGURATION =====
// Detect if running locally or on a server
const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
let API_BASE_URL = isLocalhost 
    ? 'http://localhost:3001'  // Local development
    : `${window.location.protocol}//${window.location.host}`;  // Production - use current host (Express serves on same host)

// If API_BASE_URL is still undefined or empty, default to current location
...
console.log('[NOVACOEUR] API_BASE_URL configured:', window.API_BASE_URL);
console.log('[NOVACOEUR] Current location:', window.location.href);
...
console.log('[NOVACOEUR] CONFIG disponible globalement');
